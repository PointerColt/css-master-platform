...
- [x] **Topic 1.2: Workspace Setup & Inspecting** — complete
  - Fixed unclosed `<h1 class="topic-heading">` tag
  - Fixed Question 1 missing all `data-correct="true"` attributes
  - Fixed platform-wide `.option-container` vs `.options-container` class mismatch in `exercise.css`
  - DevTools demo video **intentionally removed for now** — will be rebuilt as CSS/animation instead of screen recording
- [x] **Topic 1.3: Syntax Essentials** — complete
  - Covers: anatomy of a CSS rule, the five core selectors (universal/element/class/ID/grouping), descendant selectors, specificity (column scoring, "columns never carry over"), cascade (order as tiebreaker), common mistakes, 4-question MCQ exercise
  - New component: specificity reference table (`.spec-table`, class-based, in `main-content.css`)
  - New component: common-mistakes callout box (`.callout-mistake`, dedicated `css/mistakebox.css`) — neutral border at rest, amber border + lift + shadow on `:hover` only
...
- 1.1 Core Frontend Web Architecture — ✅ done (site Topic 1.1)
- 1.2 Workspace Configurations — ✅ done (site Topic 1.2, partial)
- 1.3 Syntax Mechanics — ✅ done (site Topic 1.3)
- 1.4 Connecting CSS to Documents (inline/internal/external) — 🔲 not yet a dedicated topic (touched briefly in site 1.2's raw draft, not formally taught)
- 1.5 DevTools Diagnostics — ✅ done (site Topic 1.2)
- 1.6 CSS Comments — 🔲 not covered

**Chapter 2: The Cascade, Specificity, and Inheritance**
- 2.1 The CSS Cascade — ✅ done (site Topic 1.3)
- 2.2 Value Inheritance — 🔲 not covered — **candidate for next topic**
- 2.3 Specificity Calculation — ✅ done (site Topic 1.3)
- 2.4 Selector Chaining (`.urgent.warning`) — 🔲 not covered — **candidate for next topic**

**Chapter 3: Basic Selectors and Colors**
- 3.1 Fundamental Selectors — ✅ done (site Topic 1.3)
- 3.2 Grouping Selectors — ✅ done (site Topic 1.3)
- 3.3 CSS Colors I — 🔲 not covered — **candidate for next topic**
- 3.4 Universal Reset — 🔲 not covered
...
- 7.1 Descendant Selectors — ✅ done (site Topic 1.3)
- 7.2 Child Selectors (`>`) — 🔲 not covered — **candidate for next topic**
- 7.3 Adjacent Sibling Selectors (`+`) — 🔲 not covered — **candidate for next topic**
...
**Next task:** Decide site-Topic 1.4 from the 🔲 candidates above (Value Inheritance, Selector Chaining, Colors, Universal Reset, Child/Sibling Selectors, Attribute Selectors) — run the per-topic NotebookLM extraction prompt once scope is picked.
